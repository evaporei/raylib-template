...
bool rlIsModelReady(Model model)
{
    bool result = false;
...
bool rlIsMaterialReady(Material material)
{
    bool result = false;
...
void rlUpdateModelAnimation(Model model, ModelAnimation anim, int frame)
{
    if ((anim.frameCount > 0) && (anim.bones != NULL) && (anim.framePoses != NULL))
    {
        ...
        for (int m = 0; m < model.meshCount; m++)
        {
            ...
            {
                TRACELOG(LOG_WARNING, "MODEL: rlUpdateModelAnimation(): Mesh %i has no connection to bones", m);
                continue;
            }
...
void rlUpdateModelAnimationBoneMatrices(Model model, ModelAnimation anim, int frame)
{
    if ((anim.frameCount > 0) && (anim.bones != NULL) && (anim.framePoses != NULL))
    {
        if (frame >= anim.frameCount) frame = frame%anim.frameCount;    
...
static Model LoadOBJ(const char *fileName)
{
    ...
    model.meshes = (Mesh*)rlMemAlloc(sizeof(Mesh) * model.meshCount);

    if (objMaterialCount > 0)
    {
        model.materialCount = objMaterialCount;
        model.materials = (Material*)rlMemAlloc(sizeof(Material) * objMaterialCount);
    }
    else // we must allocate at least one material
    {
        model.materialCount = 1;
        model.materials = (Material*)rlMemAlloc(sizeof(Material) * 1);
    }
