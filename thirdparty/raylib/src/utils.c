...
static TraceLogCallback traceLog = NULL;            // rlTraceLog callback function pointer
static LoadFileDataCallback loadFileData = NULL;    // rlLoadFileData callback function pointer
static SaveFileDataCallback saveFileData = NULL;    // rlSaveFileText callback function pointer
static LoadFileTextCallback loadFileText = NULL;    // rlLoadFileText callback function pointer
static SaveFileTextCallback saveFileText = NULL;    // rlSaveFileText callback function pointer
        ...
        if (file != NULL)
        {
            ...
            if (size > 0)
            {
                ...
                if (data != NULL)
                {
                    ...
                    if (count > 2147483647)
                    {
                        TRACELOG(LOG_WARNING, "FILEIO: [%s] File is bigger than 2147483647 bytes, avoid using rlLoadFileData()", fileName);
