...
void rlSetWindowState(unsigned int flags)
{
    ...
    // NOTE: This must be handled before FLAG_FULLSCREEN_MODE because rlToggleBorderlessWindowed() needs to get some fullscreen values if fullscreen is running
...
void rlOpenURL(const char *url)
{
    ...
    else
    {
        ...
        if (result == -1) TRACELOG(LOG_WARNING, "rlOpenURL() child process could not be created");
        RL_FREE(cmd);
    }
}
