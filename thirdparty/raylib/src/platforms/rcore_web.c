...
void rlSetWindowState(unsigned int flags)
{
    ...
    {
        TRACELOG(LOG_WARNING, "rlSetWindowState(FLAG_VSYNC_HINT) not available on target platform");
    }

    // State change: FLAG_BORDERLESS_WINDOWED_MODE
    if ((flags & FLAG_BORDERLESS_WINDOWED_MODE) > 0)
    {
        // NOTE: Window state flag updated inside rlToggleBorderlessWindowed() function
        const bool wasFullscreen = EM_ASM_INT( { if (document.fullscreenElement) return 1; }, 0);
        if (wasFullscreen)
        {
            const int canvasWidth = EM_ASM_INT( { return document.getElementById('canvas').width; }, 0);
            const int canvasStyleWidth = EM_ASM_INT( { return parseInt(document.getElementById('canvas').style.width); }, 0);
            if ((CORE.Window.flags & FLAG_FULLSCREEN_MODE) || canvasStyleWidth > canvasWidth) rlToggleBorderlessWindowed();
        }
    ...
    {
        // NOTE: Window state flag updated inside rlToggleFullscreen() function
        const bool wasFullscreen = EM_ASM_INT( { if (document.fullscreenElement) return 1; }, 0);
        if (wasFullscreen)
        {
            const int canvasWidth = EM_ASM_INT( { return document.getElementById('canvas').width; }, 0);
            const int screenWidth = EM_ASM_INT( { return screen.width; }, 0);
            if ((CORE.Window.flags & FLAG_BORDERLESS_WINDOWED_MODE) || screenWidth == canvasWidth ) rlToggleFullscreen();
        }
    ...
    {
        TRACELOG(LOG_WARNING, "rlSetWindowState(FLAG_WINDOW_UNDECORATED) not available on target platform");
    }

    // State change: FLAG_WINDOW_HIDDEN
    if ((flags & FLAG_WINDOW_HIDDEN) > 0)
    {
        TRACELOG(LOG_WARNING, "rlSetWindowState(FLAG_WINDOW_HIDDEN) not available on target platform");
    }

    // State change: FLAG_WINDOW_MINIMIZED
    if ((flags & FLAG_WINDOW_MINIMIZED) > 0)
    {
        TRACELOG(LOG_WARNING, "rlSetWindowState(FLAG_WINDOW_MINIMIZED) not available on target platform");
    }

    // State change: FLAG_WINDOW_MAXIMIZED
    if ((flags & FLAG_WINDOW_MAXIMIZED) > 0)
    {
        TRACELOG(LOG_WARNING, "rlSetWindowState(FLAG_WINDOW_MAXIMIZED) not available on target platform");
    }

    // State change: FLAG_WINDOW_UNFOCUSED
    if ((flags & FLAG_WINDOW_UNFOCUSED) > 0)
    {
        TRACELOG(LOG_WARNING, "rlSetWindowState(FLAG_WINDOW_UNFOCUSED) not available on target platform");
    }

    // State change: FLAG_WINDOW_TOPMOST
    if ((flags & FLAG_WINDOW_TOPMOST) > 0)
    {
        TRACELOG(LOG_WARNING, "rlSetWindowState(FLAG_WINDOW_TOPMOST) not available on target platform");
    }

    // State change: FLAG_WINDOW_ALWAYS_RUN
    if ((flags & FLAG_WINDOW_ALWAYS_RUN) > 0)
    {
        TRACELOG(LOG_WARNING, "rlSetWindowState(FLAG_WINDOW_ALWAYS_RUN) not available on target platform");
    }
    ...
    {
        TRACELOG(LOG_WARNING, "rlSetWindowState(FLAG_WINDOW_TRANSPARENT) not available on target platform");
    }

    // State change: FLAG_WINDOW_HIGHDPI
    if ((flags & FLAG_WINDOW_HIGHDPI) > 0)
    {
        TRACELOG(LOG_WARNING, "rlSetWindowState(FLAG_WINDOW_HIGHDPI) not available on target platform");
    }

    // State change: FLAG_WINDOW_MOUSE_PASSTHROUGH
    if ((flags & FLAG_WINDOW_MOUSE_PASSTHROUGH) > 0)
    {
        TRACELOG(LOG_WARNING, "rlSetWindowState(FLAG_WINDOW_MOUSE_PASSTHROUGH) not available on target platform");
    }

    // State change: FLAG_MSAA_4X_HINT
    if ((flags & FLAG_MSAA_4X_HINT) > 0)
    {
        TRACELOG(LOG_WARNING, "rlSetWindowState(FLAG_MSAA_4X_HINT) not available on target platform");
    }

    // State change: FLAG_INTERLACED_HINT
    if ((flags & FLAG_INTERLACED_HINT) > 0)
    {
        TRACELOG(LOG_WARNING, "rlSetWindowState(FLAG_INTERLACED_HINT) not available on target platform");
    }
}
...
void rlClearWindowState(unsigned int flags)
{
    ...
    if ((flags & FLAG_VSYNC_HINT) > 0)
    {
        TRACELOG(LOG_WARNING, "rlClearWindowState(FLAG_VSYNC_HINT) not available on target platform");
    }
    ...
    if ((flags & FLAG_WINDOW_HIDDEN) > 0)
    {
        TRACELOG(LOG_WARNING, "rlClearWindowState(FLAG_WINDOW_HIDDEN) not available on target platform");
    }

    // State change: FLAG_WINDOW_MINIMIZED
    if ((flags & FLAG_WINDOW_MINIMIZED) > 0)
    {
        TRACELOG(LOG_WARNING, "rlClearWindowState(FLAG_WINDOW_MINIMIZED) not available on target platform");
    }

    // State change: FLAG_WINDOW_MAXIMIZED
    if ((flags & FLAG_WINDOW_MAXIMIZED) > 0)
    {
        TRACELOG(LOG_WARNING, "rlClearWindowState(FLAG_WINDOW_MAXIMIZED) not available on target platform");
    }

    // State change: FLAG_WINDOW_UNDECORATED
    if ((flags & FLAG_WINDOW_UNDECORATED) > 0)
    {
        TRACELOG(LOG_WARNING, "rlClearWindowState(FLAG_WINDOW_UNDECORATED) not available on target platform");
    }

    // State change: FLAG_WINDOW_UNFOCUSED
    if ((flags & FLAG_WINDOW_UNFOCUSED) > 0)
    {
        TRACELOG(LOG_WARNING, "rlClearWindowState(FLAG_WINDOW_UNFOCUSED) not available on target platform");
    }

    // State change: FLAG_WINDOW_TOPMOST
    if ((flags & FLAG_WINDOW_TOPMOST) > 0)
    {
        TRACELOG(LOG_WARNING, "rlClearWindowState(FLAG_WINDOW_TOPMOST) not available on target platform");
    }

    // State change: FLAG_WINDOW_ALWAYS_RUN
    if ((flags & FLAG_WINDOW_ALWAYS_RUN) > 0)
    {
        TRACELOG(LOG_WARNING, "rlClearWindowState(FLAG_WINDOW_ALWAYS_RUN) not available on target platform");
    }
    ...
    if ((flags & FLAG_WINDOW_TRANSPARENT) > 0)
    {
        TRACELOG(LOG_WARNING, "rlClearWindowState(FLAG_WINDOW_TRANSPARENT) not available on target platform");
    }

    // State change: FLAG_WINDOW_HIGHDPI
    if ((flags & FLAG_WINDOW_HIGHDPI) > 0)
    {
        TRACELOG(LOG_WARNING, "rlClearWindowState(FLAG_WINDOW_HIGHDPI) not available on target platform");
    }

    // State change: FLAG_WINDOW_MOUSE_PASSTHROUGH
    if ((flags & FLAG_WINDOW_MOUSE_PASSTHROUGH) > 0)
    {
        TRACELOG(LOG_WARNING, "rlClearWindowState(FLAG_WINDOW_MOUSE_PASSTHROUGH) not available on target platform");
    }

    // State change: FLAG_MSAA_4X_HINT
    if ((flags & FLAG_MSAA_4X_HINT) > 0)
    {
        TRACELOG(LOG_WARNING, "rlClearWindowState(FLAG_MSAA_4X_HINT) not available on target platform");
    }

    // State change: FLAG_INTERLACED_HINT
    if ((flags & FLAG_INTERLACED_HINT) > 0)
    {
        TRACELOG(LOG_WARNING, "rlClearWindowState(FLAG_INTERLACED_HINT) not available on target platform");
    }
}
...
{
    TRACELOG(LOG_INFO, "rlSetGamepadMappings not implemented in rcore_web.c");

    return 0;
}

// Set gamepad vibration
void rlSetGamepadVibration(int gamepad, float leftMotor, float rightMotor)
{
