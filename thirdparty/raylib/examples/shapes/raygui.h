...
*                         REVIEWED: Local rlGetCodepointNext(), aligned with raylib
...
*                         REMOVED: rlMeasureTextEx() dependency, logic directly implemented
...
*           - int rlGetCharPressed(void);         // -- GuiTextBox(), GuiValueBox()
*
*           - void rlDrawRectangle(int x, int y, int width, int height, Color color); // -- GuiDrawRectangle()
*           - void rlDrawRectangleGradientEx(Rectangle rec, Color col1, Color col2, Color col3, Color col4); // -- GuiColorPicker()
*
*           - Font rlGetFontDefault(void);                            // -- GuiLoadStyleDefault()
*           - Font rlLoadFontEx(const char *fileName, int fontSize, int *codepoints, int codepointCount); // -- GuiLoadStyle()
*           - Texture2D rlLoadTextureFromImage(Image image);          // -- GuiLoadStyle(), required to load texture from embedded font atlas image
*           - void rlSetShapesTexture(Texture2D tex, Rectangle rec);  // -- GuiLoadStyle(), required to set shapes rec to font white rec (optimization)
*           - char *rlLoadFileText(const char *fileName);             // -- GuiLoadStyle(), required to load charset data
*           - void rlUnloadFileText(char *text);                      // -- GuiLoadStyle(), required to unload charset data
*           - const char *rlGetDirectoryPath(const char *filePath);   // -- GuiLoadStyle(), required to find charset/font file from text .rgs
*           - int *rlLoadCodepoints(const char *text, int *count);    // -- GuiLoadStyle(), required to load required font codepoints list
*           - void rlUnloadCodepoints(int *codepoints);               // -- GuiLoadStyle(), required to unload codepoints list
*           - unsigned char *rlDecompressData(const unsigned char *compData, int compDataSize, int *dataSize); // -- GuiLoadStyle()
*
*   CONTRIBUTORS:
...
#include <stdarg.h>             // Required for: va_list, va_start(), vfprintf(), va_end() [rlTextFormat()]
...
//-------------------------------------------------------------------------------
static Vector2 rlGetMousePosition(void);
...
//-------------------------------------------------------------------------------
static void rlDrawRectangle(int x, int y, int width, int height, Color color);        // -- GuiDrawRectangle()
static void rlDrawRectangleGradientEx(Rectangle rec, Color col1, Color col2, Color col3, Color col4); // -- GuiColorPicker()
//-------------------------------------------------------------------------------

// Text required functions
//-------------------------------------------------------------------------------
static Font rlGetFontDefault(void);                            // -- GuiLoadStyleDefault()
static Font rlLoadFontEx(const char *fileName, int fontSize, int *codepoints, int codepointCount); // -- GuiLoadStyle(), load font

static Texture2D rlLoadTextureFromImage(Image image);          // -- GuiLoadStyle(), required to load texture from embedded font atlas image
static void rlSetShapesTexture(Texture2D tex, Rectangle rec);  // -- GuiLoadStyle(), required to set shapes rec to font white rec (optimization)
...
//-------------------------------------------------------------------------------
static Color rlGetColor(int hexValue);                // Returns a Color struct from hexadecimal value
static int rlColorToInt(Color color);                 // Returns hexadecimal value for a Color
static bool rlCheckCollisionPointRec(Vector2 point, Rectangle rec);   // Check if point is inside rectangle
...
static void rlDrawRectangleGradientV(int posX, int posY, int width, int height, Color color1, Color color2);  // Draw rectangle vertical gradient
//-------------------------------------------------------------------------------
...
static Color GuiFade(Color color, float alpha);         // rlFade color by an alpha factor
...
int GuiGroupBox(Rectangle bounds, const char *text)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){ bounds.x, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height }, 0, BLANK, rlGetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){ bounds.x, bounds.y + bounds.height - 1, bounds.width, RAYGUI_GROUPBOX_LINE_THICK }, 0, BLANK, rlGetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){ bounds.x + bounds.width - 1, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height }, 0, BLANK, rlGetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));
...
int GuiLine(Rectangle bounds, const char *text)
{
    ...
    Color color = rlGetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR));
...
int GuiPanel(Rectangle bounds, const char *text)
{
    ...
    GuiDrawRectangle(bounds, RAYGUI_PANEL_BORDER_WIDTH, rlGetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED: LINE_COLOR)),
                     rlGetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BASE_COLOR_DISABLED : BACKGROUND_COLOR)));
    //--------------------------------------------------------------------
...
int GuiTabBar(Rectangle bounds, const char **text, int count, int *active)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (tabBounds.x < rlGetScreenWidth())
        {
            ...
            // NOTE: Only draw close button for current tab: if (rlCheckCollisionPointRec(mousePosition, tabBounds))
            int tempBorderWidth = GuiGetStyle(BUTTON, BORDER_WIDTH);
            int tempTextAlignment = GuiGetStyle(BUTTON, TEXT_ALIGNMENT);
            GuiSetStyle(BUTTON, BORDER_WIDTH, 1);
            GuiSetStyle(BUTTON, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
    ...
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){ bounds.x, bounds.y + bounds.height - 1, bounds.width, 1 }, 0, BLANK, rlGetColor(GuiGetStyle(TOGGLE, BORDER_COLOR_NORMAL)));
    //--------------------------------------------------------------------
...
int GuiScrollPanel(Rectangle bounds, const char *text, Rectangle content, Vector2 *scroll, Rectangle *view)
{
    ...
    *view = (GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)?
    ...
    {
        Vector2 mousePoint = rlGetMousePosition();
...
int GuiTextBox(Rectangle bounds, char *text, int textSize, bool editMode)
{
    ...
    if (rlIsKeyDown(KEY_LEFT) || rlIsKeyDown(KEY_RIGHT) || rlIsKeyDown(KEY_UP) || rlIsKeyDown(KEY_DOWN) || rlIsKeyDown(KEY_BACKSPACE) || rlIsKeyDown(KEY_DELETE)) autoCursorCooldownCounter++;
    else
    {
        ...
        (wrapMode == TEXT_WRAP_NONE))               // No wrap mode
    {
        ...
        if (editMode)
        {
            ...
            if ((textLength > textBoxCursorIndex) && (rlIsKeyPressed(KEY_DELETE) || (rlIsKeyDown(KEY_DELETE) && (autoCursorCooldownCounter >= RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN))))
            {
                autoCursorDelayCounter++;

                if (rlIsKeyPressed(KEY_DELETE) || (autoCursorDelayCounter%RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY) == 0)      // Delay every movement some frames
                {
                    int nextCodepointSize = 0;
            ...
            if ((textLength > 0) && (rlIsKeyPressed(KEY_BACKSPACE) || (rlIsKeyDown(KEY_BACKSPACE) && (autoCursorCooldownCounter >= RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN))))
            {
                autoCursorDelayCounter++;

                if (rlIsKeyPressed(KEY_BACKSPACE) || (autoCursorDelayCounter%RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY) == 0)      // Delay every movement some frames
                {
            ...
            if (rlIsKeyPressed(KEY_LEFT) || (rlIsKeyDown(KEY_LEFT) && (autoCursorCooldownCounter > RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN)))
            {
                autoCursorDelayCounter++;

                if (rlIsKeyPressed(KEY_LEFT) || (autoCursorDelayCounter%RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY) == 0)      // Delay every movement some frames
                {
                    int prevCodepointSize = 0;
                    rlGetCodepointPrevious(text + textBoxCursorIndex, &prevCodepointSize);
            ...
            else if (rlIsKeyPressed(KEY_RIGHT) || (rlIsKeyDown(KEY_RIGHT) && (autoCursorCooldownCounter > RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN)))
            {
                autoCursorDelayCounter++;

                if (rlIsKeyPressed(KEY_RIGHT) || (autoCursorDelayCounter%RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY) == 0)      // Delay every movement some frames
                {
                    int nextCodepointSize = 0;
                    rlGetCodepointNext(text + textBoxCursorIndex, &nextCodepointSize);
...
int GuiSpinner(Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    ...
    GuiDrawText(text, textBounds, (GuiGetStyle(SPINNER, TEXT_ALIGNMENT) == TEXT_ALIGN_RIGHT)? TEXT_ALIGN_LEFT : TEXT_ALIGN_RIGHT, rlGetColor(GuiGetStyle(LABEL, TEXT + (state*3))));
    //--------------------------------------------------------------------
...
int GuiSliderPro(Rectangle bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue, int sliderWidth)
{
    ...
    if (textLeft != NULL)
    {
        ...
        GuiDrawText(textLeft, textBounds, TEXT_ALIGN_RIGHT, rlGetColor(GuiGetStyle(SLIDER, TEXT + (state*3))));
    }
    ...
    if (textRight != NULL)
    {
        ...
        GuiDrawText(textRight, textBounds, TEXT_ALIGN_LEFT, rlGetColor(GuiGetStyle(SLIDER, TEXT + (state*3))));
    }
    //--------------------------------------------------------------------
...
int GuiProgressBar(Rectangle bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue)
{
    ...
    else
    {
        if (*value > minValue)
        {
            ...
            GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){ bounds.x, bounds.y, (int)progress.width + (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, BLANK, rlGetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
            GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){ bounds.x, bounds.y + 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height - 2 }, 0, BLANK, rlGetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
            GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){ bounds.x, bounds.y + bounds.height - 1, (int)progress.width + (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, BLANK, rlGetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
        }
        else GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){ bounds.x, bounds.y, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height }, 0, BLANK, rlGetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));

        if (*value >= maxValue) GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){ bounds.x + progress.width + 1, bounds.y, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height }, 0, BLANK, rlGetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
        else
        {
            // Draw borders not yet reached by value
            GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){ bounds.x + (int)progress.width + 1, bounds.y, bounds.width - (int)progress.width - 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, BLANK, rlGetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
            GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){ bounds.x + (int)progress.width + 1, bounds.y + bounds.height - 1, bounds.width - (int)progress.width - 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, BLANK, rlGetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
            GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){ bounds.x + bounds.width - 1, bounds.y + 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height - 2 }, 0, BLANK, rlGetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
        }
    ...
    if (textLeft != NULL)
    {
        ...
        GuiDrawText(textLeft, textBounds, TEXT_ALIGN_RIGHT, rlGetColor(GuiGetStyle(PROGRESSBAR, TEXT + (state*3))));
    }
    ...
    if (textRight != NULL)
    {
        ...
        GuiDrawText(textRight, textBounds, TEXT_ALIGN_LEFT, rlGetColor(GuiGetStyle(PROGRESSBAR, TEXT + (state*3))));
    }
    //--------------------------------------------------------------------
...
int GuiListViewEx(Rectangle bounds, const char **text, int count, int *scrollIndex, int *active, int *focus)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(bounds, GuiGetStyle(DEFAULT, BORDER_WIDTH), rlGetColor(GuiGetStyle(LISTVIEW, BORDER + state*3)), rlGetColor(GuiGetStyle(DEFAULT, BACKGROUND_COLOR)));     // Draw background

    // Draw visible items
    for (int i = 0; ((i < visibleItems) && (text != NULL)); i++)
    {
        GuiDrawRectangle(itemBounds, GuiGetStyle(LISTVIEW, LIST_ITEMS_BORDER_WIDTH), rlGetColor(GuiGetStyle(LISTVIEW, BORDER_COLOR_NORMAL)), BLANK);

        if (state == STATE_DISABLED)
        {
            if ((startIndex + i) == itemSelected) GuiDrawRectangle(itemBounds, GuiGetStyle(LISTVIEW, BORDER_WIDTH), rlGetColor(GuiGetStyle(LISTVIEW, BORDER_COLOR_DISABLED)), rlGetColor(GuiGetStyle(LISTVIEW, BASE_COLOR_DISABLED)));
        ...
        else
        {
            ...
            {
                // Draw item selected
                GuiDrawRectangle(itemBounds, GuiGetStyle(LISTVIEW, BORDER_WIDTH), rlGetColor(GuiGetStyle(LISTVIEW, BORDER_COLOR_PRESSED)), rlGetColor(GuiGetStyle(LISTVIEW, BASE_COLOR_PRESSED)));
            ...
            {
                // Draw item focused
                GuiDrawRectangle(itemBounds, GuiGetStyle(LISTVIEW, BORDER_WIDTH), rlGetColor(GuiGetStyle(LISTVIEW, BORDER_COLOR_FOCUSED)), rlGetColor(GuiGetStyle(LISTVIEW, BASE_COLOR_FOCUSED)));
...
int GuiColorBarAlpha(Rectangle bounds, const char *text, float *alpha)
{
    ...
    if (state != STATE_DISABLED)
    {
        ...
        rlDrawRectangleGradientEx(bounds, RAYGUI_CLITERAL(Color){ 255, 255, 255, 0 }, RAYGUI_CLITERAL(Color){ 255, 255, 255, 0 }, rlFade(RAYGUI_CLITERAL(Color){ 0, 0, 0, 255 }, guiAlpha), rlFade(RAYGUI_CLITERAL(Color){ 0, 0, 0, 255 }, guiAlpha));
    }
...
int GuiColorBarHue(Rectangle bounds, const char *text, float *hue)
{
    ...
    if (state != STATE_DISABLED)
    {
        ...
        // TODO: Use directly rlDrawRectangleGradientEx(bounds, color1, color2, color2, color1);
        rlDrawRectangleGradientV((int)bounds.x, (int)(bounds.y), (int)bounds.width, (int)ceilf(bounds.height/6), rlFade(RAYGUI_CLITERAL(Color){ 255, 0, 0, 255 }, guiAlpha), rlFade(RAYGUI_CLITERAL(Color){ 255, 255, 0, 255 }, guiAlpha));
        rlDrawRectangleGradientV((int)bounds.x, (int)(bounds.y + bounds.height/6), (int)bounds.width, (int)ceilf(bounds.height/6), rlFade(RAYGUI_CLITERAL(Color){ 255, 255, 0, 255 }, guiAlpha), rlFade(RAYGUI_CLITERAL(Color){ 0, 255, 0, 255 }, guiAlpha));
        rlDrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 2*(bounds.height/6)), (int)bounds.width, (int)ceilf(bounds.height/6), rlFade(RAYGUI_CLITERAL(Color){ 0, 255, 0, 255 }, guiAlpha), rlFade(RAYGUI_CLITERAL(Color){ 0, 255, 255, 255 }, guiAlpha));
        rlDrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 3*(bounds.height/6)), (int)bounds.width, (int)ceilf(bounds.height/6), rlFade(RAYGUI_CLITERAL(Color){ 0, 255, 255, 255 }, guiAlpha), rlFade(RAYGUI_CLITERAL(Color){ 0, 0, 255, 255 }, guiAlpha));
        rlDrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 4*(bounds.height/6)), (int)bounds.width, (int)ceilf(bounds.height/6), rlFade(RAYGUI_CLITERAL(Color){ 0, 0, 255, 255 }, guiAlpha), rlFade(RAYGUI_CLITERAL(Color){ 255, 0, 255, 255 }, guiAlpha));
        rlDrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 5*(bounds.height/6)), (int)bounds.width, (int)(bounds.height/6), rlFade(RAYGUI_CLITERAL(Color){ 255, 0, 255, 255 }, guiAlpha), rlFade(RAYGUI_CLITERAL(Color){ 255, 0, 0, 255 }, guiAlpha));
    }
                ...
                switch (buffer[0])
                {
                    ...
                    case 'f':
                    {
                        ...
                        if (fontFileName[0] != '\0')
                        {
                            ...
                            if (font.texture.id != rlGetFontDefault().texture.id) UnloadTexture(font.texture);
...
void GuiLoadStyleDefault(void)
{
    ...
    if (guiFont.texture.id != rlGetFontDefault().texture.id)
    {
        ...
        rlSetShapesTexture(guiFont.texture, RAYGUI_CLITERAL(Rectangle){ whiteChar.x + 1, whiteChar.y + 1, whiteChar.width - 2, whiteChar.height - 2 });
    }
}
...
static void GuiLoadStyleFromMemory(const unsigned char *fileData, int dataSize)
{
        ...
        (signature[3] == ' '))
    {
        ...
        if (fontDataSize > 0)
        {
            ...
            {
                // Compressed font atlas image data (DEFLATE), it requires rlDecompressData()
            ...
            if (font.texture.id != 0)
            {
                ...
                if ((recsDataCompressedSize > 0) && (recsDataCompressedSize != recsDataSize))
                {
                    ...
                    font.recs = (Rectangle *)rlDecompressData(recsDataCompressed, recsDataCompressedSize, &recsDataUncompSize);
...
static int GetTextWidth(const char *text)
{
    ...
    if ((text != NULL) && (text[0] != '\0'))
    {
        ...
        // Custom rlMeasureText() implementation
                ...
                if ((codepoint != ' ') && (codepoint != '\t'))      // Do not draw codepoints with no glyph
                {
                    if (wrapMode == TEXT_WRAP_NONE)
                    {
                        ...
                        if (textSizeX > textBounds.width)
                        {
                            ...
                            {
                                rlDrawTextCodepoint(guiFont, codepoint, RAYGUI_CLITERAL(Vector2){ textBoundsPosition.x + textOffsetX, textBoundsPosition.y + textOffsetY }, (float)GuiGetStyle(DEFAULT, TEXT_SIZE), GuiFade(tint, guiAlpha));
                            }
                            else if (!textOverflow)
                            {
                                ...
                                {
                                    rlDrawTextCodepoint(guiFont, '.', RAYGUI_CLITERAL(Vector2){ textBoundsPosition.x + textOffsetX + j, textBoundsPosition.y + textOffsetY }, (float)GuiGetStyle(DEFAULT, TEXT_SIZE), GuiFade(tint, guiAlpha));
                                }
                            }
                        }
                        else
...
static void GuiTooltip(Rectangle controlRec)
{
    ...
    {
        Vector2 textSize = rlMeasureTextEx(GuiGetFont(), guiTooltipPtr, (float)GuiGetStyle(DEFAULT, TEXT_SIZE), (float)GuiGetStyle(DEFAULT, TEXT_SPACING));
...
static int GuiScrollBar(Rectangle bounds, int value, int minValue, int maxValue)
{
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        else if (rlCheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            if (isVertical)
            {
                if (rlIsKeyDown(KEY_DOWN)) value += 5;
                else if (rlIsKeyDown(KEY_UP)) value -= 5;
            }
            else
            {
                if (rlIsKeyDown(KEY_RIGHT)) value += 5;
                else if (rlIsKeyDown(KEY_LEFT)) value -= 5;
            }
            */
        }
...
static Color rlGetColor(int hexValue)
{
    Color color;
...
static int rlColorToInt(Color color)
{
    return (((int)color.r << 24) | ((int)color.g << 16) | ((int)color.b << 8) | (int)color.a);
}

// Check if point is inside rectangle
static bool rlCheckCollisionPointRec(Vector2 point, Rectangle rec)
{
...
static void rlDrawRectangleGradientV(int posX, int posY, int width, int height, Color color1, Color color2)
{
