...
*
*           - void rlDrawRectangle(int x, int y, int width, int height, rlColor color); // -- GuiDrawRectangle()
*           - void rlDrawRectangleGradientEx(rlRectangle rec, rlColor col1, rlColor col2, rlColor col3, rlColor col4); // -- GuiColorPicker()
*
*           - rlFont rlGetFontDefault(void);                            // -- GuiLoadStyleDefault()
*           - rlFont rlLoadFontEx(const char *fileName, int fontSize, int *codepoints, int codepointCount); // -- GuiLoadStyle()
*           - Texture2D rlLoadTextureFromImage(rlImage image);          // -- GuiLoadStyle(), required to load texture from embedded font atlas image
*           - void rlSetShapesTexture(Texture2D tex, rlRectangle rec);  // -- GuiLoadStyle(), required to set shapes rec to font white rec (optimization)
...
//----------------------------------------------------------------------------------
    ...
    typedef struct rlFont {
        ...
        Texture2D texture;      // rlTexture atlas containing the glyphs
...
RAYGUIAPI int GuiColorPanelHSV(rlRectangle bounds, const char *text, rlVector3 *colorHsv);                 // rlColor Panel control that updates Hue-Saturation-Value color value, used by GuiColorPickerHSV()
//----------------------------------------------------------------------------------------------------------
...
static Texture2D rlLoadTextureFromImage(rlImage image);          // -- GuiLoadStyle(), required to load texture from embedded font atlas image
static void rlSetShapesTexture(Texture2D tex, rlRectangle rec);  // -- GuiLoadStyle(), required to set shapes rec to font white rec (optimization)
...
//-------------------------------------------------------------------------------
static rlColor rlGetColor(int hexValue);                // Returns a rlColor struct from hexadecimal value
static int rlColorToInt(rlColor color);                 // Returns hexadecimal value for a rlColor
...
static rlColor GuiFade(rlColor color, float alpha);         // rlFade color by an alpha factor
...
int GuiGroupBox(rlRectangle bounds, const char *text)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height }, 0, BLANK, rlGetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));
    GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x, bounds.y + bounds.height - 1, bounds.width, RAYGUI_GROUPBOX_LINE_THICK }, 0, BLANK, rlGetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));
    GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x + bounds.width - 1, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height }, 0, BLANK, rlGetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));
...
int GuiLine(rlRectangle bounds, const char *text)
{
    ...
    rlColor color = rlGetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR));
...
int GuiProgressBar(rlRectangle bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue)
{
    ...
    else
    {
        ...
        else GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x, bounds.y, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height }, 0, BLANK, rlGetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));

        if (*value >= maxValue) GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x + progress.width + 1, bounds.y, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height }, 0, BLANK, rlGetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
        else
        {
            // Draw borders not yet reached by value
            GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x + (int)progress.width + 1, bounds.y, bounds.width - (int)progress.width - 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, BLANK, rlGetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
            GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x + (int)progress.width + 1, bounds.y + bounds.height - 1, bounds.width - (int)progress.width - 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, BLANK, rlGetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
            GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x + bounds.width - 1, bounds.y + 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height - 2 }, 0, BLANK, rlGetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
        }
...
// rlColor Panel control - rlColor (RGBA) variant.
int GuiColorPanel(rlRectangle bounds, const char *text, rlColor *color)
{
    ...
    // This is required, because the rlColor->HSV->rlColor conversion has precision errors.
    // Thus the assignment from HSV to rlColor should only be made, if the HSV has a new user-entered value.
...
// rlColor Panel control - HSV variant
int GuiColorPanelHSV(rlRectangle bounds, const char *text, rlVector3 *colorHsv)
{
    int result = 0;
    GuiState state = guiState;
